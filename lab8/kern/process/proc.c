...
load_icode(int fd, int argc, char **kargv)
{
    ...
    struct proghdr *ph_base = (struct proghdr *)kmalloc(ph_size);
    if (ph_base == NULL)
    {
        goto bad_elf_cleanup_pgdir;
    }
    if ((ret = load_icode_read(fd, ph_base, ph_size, elf.e_phoff)) != 0)
    {
        goto bad_free_ph;
    }

    uint32_t vm_flags, perm;
    struct proghdr *ph = ph_base, *ph_end = ph_base + elf.e_phnum;
    for (; ph < ph_end; ph++)
    {
        ...
        if (ph->p_flags & ELF_PF_X)
            vm_flags |= VM_EXEC, perm |= PTE_X;
        if (ph->p_flags & ELF_PF_W)
            vm_flags |= VM_WRITE, perm |= (PTE_W | PTE_R);
        if (ph->p_flags & ELF_PF_R)
            vm_flags |= VM_READ, perm |= PTE_R;
        ...
        if (start < end)
        {
            ...
            if (page != NULL)
            {
                zero_sz = PGSIZE - (start - la);
                if (end < la + PGSIZE)
                {
                    zero_sz -= (la + PGSIZE) - end;
                }
                memset(page2kva(page) + (start - la), 0, zero_sz);
                start = la + PGSIZE;
                la += PGSIZE;
            }
            while (start < end)
            {
                ...
                memset(page2kva(page), 0, zero_sz);
                start += zero_sz;
                la += PGSIZE;
            }
        }
    ...
    uargv[argc] = 0;
    sp = ROUNDDOWN(sp - (argc + 1) * sizeof(uintptr_t), sizeof(uintptr_t));
    if (!copy_to_user(mm, (void *)sp, uargv, (argc + 1) * sizeof(uintptr_t)))
    {
        ret = -E_INVAL;
        goto bad_cleanup_mmap;
    }
